...
export class UnipileDirectSync {
  ...
  async testConnection(): Promise<boolean> {
    try {
      if (!this.apiKey) {
        ...
        console.error("ERROR:", 'Unipile API key not configured');
        return false;
      }
  ...
  async syncAllMessages(): Promise<void> {
    try {
      ...
      if (!user) {
        console.error("ERROR:", 'Please sign in to sync messages');
        return;
      }

      console.info("INFO:", 'Connecting to Unipile API...');
      
      // Test connection first
      const isConnected = await this.testConnection();
      if (!isConnected) {
        console.error("ERROR:", 'Failed to connect to Unipile API. Please check your API key.');
        return;
      }
      ...
      if (accounts.length === 0) {
        ...
        if (storedAccounts) {
          ...
          console.warn("WARNING:", 'No LinkedIn accounts connected. Please connect an account first.');
        }
        return;
      }

      console.info("INFO:", `Found ${accounts.length} LinkedIn account(s). Syncing messages...`);
      ...
      console.log("SUCCESS:", 'All LinkedIn messages synced successfully!');
    } catch (error) {
      console.error('Sync error:', error);
      console.error("ERROR:", 'Failed to sync messages. Check console for details.');
    }
  }
  ...
  setApiKey(apiKey: string): void {
    ...
    console.log("SUCCESS:", 'Unipile API key saved');
  }
}
