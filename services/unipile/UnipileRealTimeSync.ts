...
export class UnipileRealTimeSync {
  ...
  startAutoSync(intervalMinutes: number = 30): void {
    if (!this.isConfigured()) {
      console.error("ERROR:", 'Unipile API not configured');
      return;
    }
    ...
    console.log("SUCCESS:", `Auto-sync started (every ${intervalMinutes} minutes)`);
  }
  ...
  stopAutoSync(): void {
    if (this.syncInterval) {
      ...
      console.info("INFO:", 'Auto-sync stopped');
    }
  }
  ...
  async syncAll(): Promise<void> {
    ...
    try {
      ...
      if (accounts.length === 0) {
        ...
        console.warn("WARNING:", 'No LinkedIn accounts connected');
        return;
      }
      ...
      if (totalMessages > 0 || totalContacts > 0) {
        console.log("SUCCESS:", `Synced ${totalMessages} messages and ${totalContacts} contacts`);
      } else {
        console.info("INFO:", 'No new data to sync');
      }

    } catch (error) {
      console.error('Sync error:', error);
      this.status.errors.push(error instanceof Error ? error.message : 'Unknown error');
      console.error("ERROR:", 'Sync failed. Check console for details.');
    } finally {
