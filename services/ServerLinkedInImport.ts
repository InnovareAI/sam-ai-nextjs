...
export class ServerLinkedInImport {
  ...
  private showServerImportResults(result: ServerImportResult) {
    if (result.success && result.contactsImported > 0) {
      ...
      console.log("SUCCESS:", `🎉 Server import successful! ${result.contactsImported} LinkedIn contacts imported!`, {
        duration: 6000
      });
      ...
      if (qualityDetails.length > 0) {
        console.info("INFO:", `🔗 Network: ${qualityDetails.join(', ')} connections`, {
          duration: 5000
        });
      }
      ...
      if (result.quality.withJobTitles > 0 || result.quality.withProfiles > 0) {
        console.info("INFO:", `✅ Quality: ${result.quality.withJobTitles} with titles, ${result.quality.withProfiles} with profiles`, {
          duration: 5000
        });
      }

      // Processing time
      console.info("INFO:", `⏱️ Server processing completed in ${(result.processingTime / 1000).toFixed(1)} seconds`, {
        duration: 4000
      });

      // Server source info
      console.info("INFO:", `🖥️ Processed server-side via ${result.source} (continues even if you close the browser)`, {
        duration: 5000
      });

    } else if (result.success && result.contactsImported === 0) {
      console.warn("WARNING:", '🔍 Server import completed but no new contacts were found', {
        duration: 5000
      });
    } else {
      console.error("ERROR:", `❌ Server import failed: ${result.errors.join(', ')}`, {
        duration: 6000
      });
    }
    ...
    if (result.warnings.length > 0) {
      console.warn("WARNING:", `⚠️ ${result.warnings.length} warnings: ${result.warnings.slice(0, 2).join(', ')}${result.warnings.length > 2 ? '...' : ''}`, {
        duration: 6000
      });
    }
  }
