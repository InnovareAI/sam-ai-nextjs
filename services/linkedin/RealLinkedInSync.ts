...
export class RealLinkedInSync {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (!this.apiKey) {
        ...
        console.error("ERROR:", 'Please configure Unipile API key in settings');
        return false;
      }
  ...
  async syncInboxMessages(): Promise<number> {
    try {
      ...
      if (totalSynced > 0) {
        console.log("SUCCESS:", `Synced ${totalSynced} new messages from LinkedIn`);
      }

      return totalSynced;
    } catch (error) {
      console.error('Inbox sync error:', error);
      console.error("ERROR:", 'Failed to sync LinkedIn inbox');
      return 0;
    }
  }
  ...
  async syncContacts(): Promise<number> {
    try {
      ...
      if (totalSynced > 0) {
        console.log("SUCCESS:", `Synced ${totalSynced} LinkedIn contacts`);
      }

      return totalSynced;
    } catch (error) {
      console.error('Contacts sync error:', error);
      console.error("ERROR:", 'Failed to sync LinkedIn contacts');
      return 0;
    }
  }
