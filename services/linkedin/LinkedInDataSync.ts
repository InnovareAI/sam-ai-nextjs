  ...
  async syncAllData(userId: string): Promise<void> {
    try {
      ...
      if (accounts.length === 0) {
        ...
        console.info("INFO:", 'No LinkedIn accounts connected. Please connect an account first.');
        return;
      }
  ...
  private async syncAccountData(account: any): Promise<void> {
    try {
      ...
      if (hasUnipileKey && account.unipileAccountId) {
        ...
        console.error("ERROR:", 'Unipile API not configured. Cannot sync LinkedIn data.');
      }
