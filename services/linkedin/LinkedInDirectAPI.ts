...
export class LinkedInDirectAPI {
  ...
  async initiateOAuth(): Promise<void> {
    ...
    if (!clientId) {
      console.error("ERROR:", 'LinkedIn Client ID not configured. Please add VITE_LINKEDIN_CLIENT_ID to environment variables.');
      return;
    }
  ...
  async handleOAuthCallback(code: string): Promise<boolean> {
    try {
      ...
      console.error("ERROR:", 'Failed to connect LinkedIn account');
      return false;
    }
  }
  ...
  async fetchMessages(): Promise<any[]> {
    if (!this.accessToken) {
      console.error("ERROR:", 'Please connect your LinkedIn account first');
      return [];
    }
    ...
    try {
      ...
      console.error("ERROR:", 'Unable to fetch LinkedIn messages. API access may be restricted.');
      return [];
    }
  }
  ...
  async syncToDatabase(): Promise<void> {
    try {
      ...
      if (!user) {
        console.error("ERROR:", 'Please sign in to sync data');
        return;
      }
      ...
      if (messages.length === 0) {
        console.warn("WARNING:", 'No LinkedIn data to sync. LinkedIn API access may be limited.');
        return;
      }
      ...
      console.error("ERROR:", 'Failed to sync LinkedIn data');
    }
  }
