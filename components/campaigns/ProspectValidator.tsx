...
export function ProspectValidator({ prospects, onProspectsChange, campaignType }: ProspectValidatorProps) {
  ...
  const validateAllProspects = useCallback(async () => {
    ...
    try {
      ...
      if (readyForCampaign) {
        console.log("SUCCESS:", 'All prospects validated successfully!');
      } else if (prospectsWithErrors > 0) {
        console.warn("WARNING:", `${prospectsWithErrors} prospects have errors that need fixing`);
      }

    } catch (error) {
      console.error('Validation error:', error);
      console.error("ERROR:", 'Failed to validate prospects');
    } finally {
  ...
  const exportValidationReport = () => {
    ...
    console.log("SUCCESS:", 'Validation report downloaded');
  };
