...
export function AddPeopleTab({ selectedPeople, onPeopleChange, campaignType }: AddPeopleTabProps) {
  ...
  const handleFileSelect = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith('.csv')) {
      console.error("ERROR:", 'Please select a CSV file');
      return;
    }
  ...
  const processCsvFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (errors.length === 0) {
          console.log("SUCCESS:", `Successfully parsed ${prospects.length} prospects from CSV`);
        } else {
          console.warn("WARNING:", `Parsed ${prospects.length} valid prospects with ${errors.length} errors`);
        }
  ...
  const addCsvProspects = useCallback(() => {
    if (csvData.length === 0) {
      console.error("ERROR:", 'No valid prospects to add');
      return;
    }
    ...
    if (newProspects.length === 0) {
      console.warn("WARNING:", 'All prospects from CSV are already added');
      return;
    }
    ...
    console.log("SUCCESS:", `Added ${newProspects.length} new prospects from CSV`);
  ...
  const extractFromSearchUrl = useCallback(async () => {
    if (!searchUrl.trim()) {
      console.error("ERROR:", 'Please enter a LinkedIn or Sales Navigator search URL');
      return;
    }
    ...
    if (userQuota && estimatedCount > userQuota.remaining) {
      console.error("ERROR:", `Cannot extract ${estimatedCount} contacts. You have ${userQuota.remaining} contacts remaining in your monthly quota.`);
      return;
    }
    ...
    try {
      ...
      if (!user) {
        console.error("ERROR:", 'Please log in to use the quota-managed extraction system');
        return;
      }
      ...
      if (!extractionResult.success) {
        if (extractionResult.quotaExceeded) {
          ...
          console.error("ERROR:", `Quota Error: ${extractionResult.errors[0]}`);
        } else {
          setUrlExtractionError(extractionResult.errors.join(', '));
          console.error("ERROR:", `Extraction failed: ${extractionResult.errors[0]}`);
        }
        return;
      }
      ...
      if (newProspects.length > 0) {
        ...
        console.warn("WARNING:", 'All extracted prospects are already in your list');
      } else {
        console.warn("WARNING:", 'No prospects were extracted from the URL');
      }
      ...
      console.error("ERROR:", 'Failed to extract prospects from URL');
    } finally {
  ...
  const addManualProspect = useCallback(() => {
    ...
    if (existingEmails.has(email.toLowerCase())) {
      console.warn("WARNING:", 'This prospect is already added');
      return;
    }
    ...
    console.log("SUCCESS:", 'Prospect added successfully');
  ...
  const removeProspect = useCallback((index: number) => {
    ...
    onPeopleChange(updatedPeople);
    console.log("SUCCESS:", 'Prospect removed');
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    console.log("SUCCESS:", 'Prospects exported successfully');
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    console.log("SUCCESS:", 'Search saved successfully');
                  }}
                >
          ...
          <div className="flex items-center justify-between">
            ...
            <Button 
              ...
              onClick={() => {
                ...
                console.log("SUCCESS:", 'CSV template downloaded');
              }}
            >
