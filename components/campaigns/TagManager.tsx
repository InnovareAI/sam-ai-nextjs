...
export default function TagManager() {
  ...
  const saveTag = async () => {
    if (!newTag.label || !newTag.value) {
      console.error("ERROR:", 'Tag name and value are required');
      return;
    }

    // Check for duplicates
    if (tags.some(t => t.value === newTag.value)) {
      console.error("ERROR:", 'A tag with this value already exists');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        console.error("ERROR:", 'Failed to save tag');
      } else {
        setTags([...tags, newTag]);
        ...
        console.log("SUCCESS:", 'Tag created successfully');
      }
    } catch (error) {
  ...
  const updateTag = async () => {
    ...
    try {
      ...
      if (isDefault) {
        console.error("ERROR:", 'Cannot edit default system tags');
        return;
      }
      ...
      if (error) {
        ...
        console.error("ERROR:", 'Failed to update tag');
      } else {
        setTags(tags.map(t => t.value === editingTag.value ? editingTag : t));
        setShowEditModal(false);
        setEditingTag(null);
        console.log("SUCCESS:", 'Tag updated successfully');
      }
    } catch (error) {
  ...
  const deleteTag = async (tag: TagData) => {
    ...
    if (isDefault) {
      console.error("ERROR:", 'Cannot delete default system tags');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        console.error("ERROR:", 'Failed to delete tag');
      } else {
