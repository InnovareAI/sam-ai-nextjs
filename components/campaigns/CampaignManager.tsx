...
export function CampaignManager({
    ...
    try {
      ...
      if (newStatus === 'active' && campaign.status === 'draft') {
        ...
        if (campaign.progress.total === 0) {
          console.error("ERROR:", 'Cannot activate campaign with no prospects');
          return;
        }
      }
      ...
      console.log("SUCCESS:", statusMessages[newStatus] || 'Campaign updated');
    } catch (error) {
      console.error("ERROR:", 'Failed to update campaign status');
    }
  };
  ...
  const handlePriorityChange = async (campaignId: string, newPriority: Campaign['priority']) => {
    try {
      ...
      console.log("SUCCESS:", 'Campaign priority updated');
    } catch (error) {
      console.error("ERROR:", 'Failed to update campaign priority');
    }
  };
