...
export function AutoSyncControl({ workspaceId, accountId: propAccountId }: AutoSyncControlProps) {
  ...
  const initializeWorkspaceService = async () => {
    try {
      // TODO: Implement single-user Unipile config
      // const config = await workspaceUnipile.initialize(workspaceId);
      const config = { account_id: 'demo', linkedin_connected: false };
  ...
  const handleToggleSync = async () => {
    ...
    try {
      if (!isEnabled) {
        ...
        if (result.success) {
          setIsEnabled(true);
          console.log("SUCCESS:", '🚀 Auto-sync enabled! Contacts will sync every ' + interval + ' minutes');
        ...
        setIsEnabled(false);
        console.info("INFO:", 'Auto-sync disabled');
        setNextSync(null);
      }
    } catch (error) {
      console.error('Error toggling sync:', error);
      console.error("ERROR:", 'Failed to update sync settings');
      setSyncStatus('error');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleIntervalChange = async (newInterval: string) => {
    ...
    if (isEnabled) {
      ...
      try {
        ...
        console.log("SUCCESS:", 'Sync interval updated to ' + intervalNum + ' minutes');
        setNextSync(new Date(Date.now() + intervalNum * 60000));
      } catch (error) {
        console.error('Error updating interval:', error);
        console.error("ERROR:", 'Failed to update sync interval');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleManualSync = async () => {
    if (!accountId) {
      console.error("ERROR:", 'No account ID available. Please check workspace configuration.');
      return;
    }

    setSyncStatus('syncing');
    console.info("INFO:", 'Starting sync with Unipile LinkedIn API...');
    try {
      // Use centralized workspace service with proper account ID
      // TODO: Implement single-user Unipile config
      // const config = await workspaceUnipile.getConfig();
      const config = { account_id: 'demo' };
      ...
      if (syncOptions.contacts || syncOptions.messages) {
        if (!linkedInConnected || !config.linkedin_connected) {
          console.warn("WARNING:", 'LinkedIn not connected. Please complete onboarding to sync contacts.');
          setSyncStatus('error');
          return;
        }
        ...
        if (result.totalContacts > 0) {
          ...
          console.log("SUCCESS:", `🎉 Synced ${result.totalContacts} LinkedIn contacts${detailText}!`);
          
          // Show data source breakdown
          if (result.unipileContacts > 0 && result.linkedinAPIContacts > 0) {
            console.info("INFO:", `📊 Sources: ${result.unipileContacts} from chats, ${result.linkedinAPIContacts} from LinkedIn API`);
          } else if (result.unipileContacts > 0) {
            console.info("INFO:", `📱 ${result.unipileContacts} contacts from LinkedIn chats/messages`);
          } else if (result.linkedinAPIContacts > 0) {
            console.info("INFO:", `🔗 ${result.linkedinAPIContacts} contacts from LinkedIn API`);
          }
          
          // Show quality metrics
          if (result.withJobTitles > 0) {
            console.info("INFO:", `✅ Quality: ${result.withJobTitles} with job titles, ${result.withProfiles} with LinkedIn profiles`);
          }
          
          // Show any errors
          if (result.errors.length > 0) {
            console.warn('Sync warnings:', result.errors);
            console.warn("WARNING:", `⚠️ ${result.errors.length} warnings during sync (check console for details)`);
          }
        } else {
          // Fallback to basic Unipile sync
          // TODO: Implement single-user contact sync
          // const fallbackResult = await workspaceUnipile.syncContacts(200);
          const fallbackResult = { contactsSynced: 0 };
          if (fallbackResult.contactsSynced > 0) {
            console.log("SUCCESS:", `✅ Synced ${fallbackResult.contactsSynced} contacts via fallback method`);
          } else {
      ...
      if (syncOptions.emails || syncOptions.calendar) {
        ...
        if (message.length > 0) {
          console.log("SUCCESS:", `Synced: ${message.join(', ')}`);
        }
      }
      ...
      setSyncStatus('error');
      console.error("ERROR:", 'Sync failed. Please try again.');
    }
  };
