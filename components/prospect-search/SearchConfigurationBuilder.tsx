...
export function SearchConfigurationBuilder({
  ...
  useEffect(() => {
    const generateUrl = () => {
      ...
      try {
        ...
        console.error("ERROR:", 'Failed to generate search URL');
      } finally {
  ...
  const applyPreset = (presetId: string) => {
    ...
    if (preset) {
      ...
      console.log("SUCCESS:", `Applied "${preset.name}" preset`);
    }
  };
  ...
  const copyUrl = () => {
    if (urlResult?.url) {
      ...
      console.log("SUCCESS:", 'URL copied to clipboard');
    }
  };
  ...
  const resetConfiguration = () => {
    ...
    setSelectedPreset('');
    setCustomKeywords('');
    console.info("INFO:", 'Configuration reset');
  };
