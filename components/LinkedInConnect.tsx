...
export function LinkedInConnect({ onConnect }: LinkedInConnectProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      console.error("ERROR:", 'Failed to connect LinkedIn. Please try again.');
    } finally {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (linkedInAccount) {
          ...
          console.error("ERROR:", 'LinkedIn connection not found. Please try again.');
        }
      }
    } catch (error) {
      console.error('Error checking connection status:', error);
      console.error("ERROR:", 'Failed to verify LinkedIn connection');
    }
  };
  ...
  const handleConnectionSuccess = async (accountId: string) => {
    ...
    try {
      ...
      if (workspaceId) {
        ...
        console.log("SUCCESS:", 'LinkedIn connected! Smart sync enabled for optimal performance', {
          duration: 5000
        });
      }
    } catch (error) {
  ...
  const handleManualConnect = () => {
    if (!accountId.trim()) {
      console.error("ERROR:", 'Please enter your Unipile Account ID');
      return;
    }
