...
export function MyNetwork({ className }: MyNetworkProps) {
  ...
  const loadNetworkData = async () => {
    try {
      ...
      console.error("ERROR:", 'Failed to load network data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleWithdrawRequest = async (requestId: string) => {
    try {
      ...
      console.log("SUCCESS:", 'Connection request withdrawn successfully');
      
      // Update local state
      setPendingRequests(prev => prev.filter(req => req.id !== requestId));
      setStats(prev => ({ ...prev, pending_requests: prev.pending_requests - 1 }));
    } catch (error) {
      console.error("ERROR:", 'Failed to withdraw connection request');
    }
  };

  const handleBulkWithdraw = async () => {
    if (selectedConnections.length === 0) {
      console.error("ERROR:", 'No requests selected');
      return;
    }

    try {
      // Bulk withdraw selected requests
      console.log("SUCCESS:", `${selectedConnections.length} connection requests withdrawn`);
      ...
      console.error("ERROR:", 'Failed to withdraw connection requests');
    }
  };
  ...
  const handleAddToFavorites = async (connectionId: string) => {
    try {
      ...
      console.error("ERROR:", 'Failed to update favorites');
    }
  };
  ...
  const handleExportConnections = () => {
    ...
    console.log("SUCCESS:", 'Connections exported successfully');
  };
