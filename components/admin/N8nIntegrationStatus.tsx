...
export function N8nIntegrationStatus() {
  ...
  const testWorkflowTrigger = async (mode: 'inbound' | 'outbound' | 'unified', stage: string) => {
    try {
      ...
      console.log("SUCCESS:", `Workflow triggered successfully! Execution ID: ${result.executionId || 'N/A'}`);
      loadIntegrationStatus(); // Refresh status
    } catch (error) {
      console.error("ERROR:", `Workflow trigger failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
