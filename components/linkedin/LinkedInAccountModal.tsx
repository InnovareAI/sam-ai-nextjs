...
export function LinkedInAccountModal({ 
  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      ...
      if (!formData.country) {
        console.error("ERROR:", 'Please select a country for your proxy');
        return;
      }
    }
                      ...
                      <Button 
                        onClick={() => {
                          if (formData.twoFactorSecret.length === 32) {
                            ...
                            console.log("SUCCESS:", `Verification code: ${code}`);
                          } else {
                            console.error("ERROR:", 'Please enter a valid 32-character secret key');
                          }
                        }}
