...
export function WorkspaceSetup({ userId, onComplete }: WorkspaceSetupProps) {
  ...
  const handleConnectLinkedIn = async () => {
    ...
    try {
      ...
      console.error("ERROR:", 'Failed to connect LinkedIn. You can set this up later in Settings.');
  ...
  const handleConnectionSuccess = async (accountId: string) => {
    try {
      ...
      console.log("SUCCESS:", 'LinkedIn connected successfully!');
      ...
      console.error("ERROR:", 'Failed to save LinkedIn connection');
    }
  };
  ...
  const triggerInitialSync = async (accountId: string, workspaceId: string) => {
    try {
      ...
      if (response.ok) {
        console.log("SUCCESS:", 'Initial sync started! Your contacts will appear shortly.');
      }
    } catch (error) {
