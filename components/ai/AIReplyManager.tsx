...
export function AIReplyManager({ className }: AIReplyManagerProps) {
  ...
  const loadReplyData = async () => {
    try {
      ...
      console.error("ERROR:", 'Failed to load reply management data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAIAnalysis = async (messageId: string) => {
    try {
      ...
      console.log("SUCCESS:", 'AI analysis initiated - results in 30 seconds');
      ...
      console.error("ERROR:", 'Failed to analyze message');
    } finally {
  ...
  const handleSendReply = async (messageId: string, content: string) => {
    try {
      ...
      console.log("SUCCESS:", `Reply sent via ${message.channel.toUpperCase()}`);
  ...
  const handleAutoReply = async (messageId: string) => {
    try {
      ...
      console.error("ERROR:", 'Failed to send auto-reply');
    }
  };
