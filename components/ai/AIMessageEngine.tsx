...
export function AIMessageEngine({ className }: AIMessageEngineProps) {
  ...
  const loadAIMessageData = async () => {
    try {
      ...
      console.error("ERROR:", 'Failed to load message data');
    }
  };

  const handleGenerateMessage = async () => {
    if (!prospectName || !prospectCompany) {
      console.error("ERROR:", 'Prospect name and company are required');
      return;
    }
    ...
    try {
      ...
      console.log("SUCCESS:", `AI generated ${newMessages.length} personalized messages!`);

    } catch (error) {
      console.error("ERROR:", 'Failed to generate messages');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleCopyMessage = (content: string) => {
    ...
    console.log("SUCCESS:", 'Message copied to clipboard');
  };

  const handleSendMessage = async (messageId: string) => {
    try {
      // This would integrate with n8n workflows or direct API calls
      console.log("SUCCESS:", 'Message queued for sending via SAM AI automation');
    } catch (error) {
