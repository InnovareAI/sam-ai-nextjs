...
export function useCampaigns(workspaceId: string) {
  ...
  const createCampaignMutation = useMutation({
    ...
    onError: (error) => {
      console.error("ERROR:", 'Failed to create campaign: ' + (error as Error).message);
    }
  });
  ...
  const updateCampaignMutation = useMutation({
    ...
    onSuccess: (updatedCampaign) => {
      ...
      console.log("SUCCESS:", 'Campaign updated successfully');
    },
    onError: (error) => {
      console.error("ERROR:", 'Failed to update campaign: ' + (error as Error).message);
    }
  });
...
export function useLeads(workspaceId: string, filters?: {
  ...
  const upsertLeadMutation = useMutation({
    ...
    onError: (error) => {
      console.error("ERROR:", 'Failed to update lead: ' + (error as Error).message);
    }
  });
  ...
  const batchCreateLeadsMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      console.log("SUCCESS:", `Created ${result.created} leads${result.errors.length > 0 ? ` with ${result.errors.length} errors` : ''}`);
    },
    onError: (error) => {
      console.error("ERROR:", 'Failed to create leads: ' + (error as Error).message);
    }
  });
...
export function useCampaignAssignments(campaignId?: string, filters?: {
  ...
  const assignLeadsMutation = useMutation({
    mutationFn: async ({ 
      ...
      if (validationResult.warnings.length > 0) {
        console.warn("WARNING:", `Proceeding with warnings: ${validationResult.warnings.slice(0, 2).join(', ')}`);
      }
    ...
    onSuccess: (result) => {
      ...
      if (result.assigned > 0) {
        console.log("SUCCESS:", `Successfully assigned ${result.assigned} leads to campaign`);
      }
      if (result.failed > 0) {
        console.warn("WARNING:", `${result.failed} leads failed assignment. Check validation requirements.`);
      }
    },
    onError: (error) => {
      console.error("ERROR:", 'Assignment failed: ' + (error as Error).message);
    }
  });
  ...
  const updateAssignmentMutation = useMutation({
    ...
    onSuccess: (updatedAssignment) => {
      ...
      console.log("SUCCESS:", 'Assignment status updated');
    },
    onError: (error) => {
      console.error("ERROR:", 'Failed to update status: ' + (error as Error).message);
    }
  });
...
export function useLeadEnrichment() {
  ...
  const enrichLeadMutation = useMutation({
    ...
    onSuccess: (result, variables) => {
      if (result.success) {
        ...
        console.log("SUCCESS:", `Enriched ${result.fields_enriched.length} fields for lead`);
      } else {
        console.error("ERROR:", `Enrichment failed: ${result.errors.join(', ')}`);
      }
    }
  });
  ...
  const bulkEnrichMutation = useMutation({
    ...
    onError: (error) => {
      console.error("ERROR:", 'Bulk enrichment failed: ' + (error as Error).message);
    }
  });
  ...
  const autoEnrichForValidationMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.enriched_count > 0) {
        console.log("SUCCESS:", `Auto-enriched ${result.enriched_count} leads for better validation`);
      }
      if (result.errors.length > 0) {
        console.warn("WARNING:", `Some enrichments failed: ${result.errors.slice(0, 2).join(', ')}`);
      }
    }
  });
  ...
  const getEnrichmentStatus = useCallback(async (leadIds: string[]) => {
    try {
      ...
      console.error("ERROR:", 'Failed to get enrichment status');
      return {};
    }
  }, []);
...
export function useCampaignManager(workspaceId: string, userId: string) {
  ...
  const smartAssignLeads = useCallback(async (
    ...
    try {
      ...
      if (auto_enrich) {
        console.info("INFO:", 'Enriching leads for better validation...');
      ...
      console.error("ERROR:", 'Smart assignment failed: ' + (error as Error).message);
      throw error;
    }
