...
export function useLocalTemplates() {
  ...
  const saveTemplates = useCallback((templatesToSave: LocalTemplate[]) => {
    try {
      ...
      console.error("ERROR:", 'Failed to save templates');
      return false;
    }
  }, []);
  ...
  const scheduleAutoSave = useCallback((templatesToSave: LocalTemplate[]) => {
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      ...
      if (saved) {
        console.log("SUCCESS:", 'Templates auto-saved', {
          duration: 2000,
  ...
  const updateTemplate = useCallback(async (
    ...
    try {
      ...
      console.error("ERROR:", 'Failed to update template');
      throw err;
    }
  ...
  const deleteTemplate = useCallback(async (templateId: string) => {
    try {
      ...
      console.log("SUCCESS:", 'Template deleted successfully!');
    } catch (err) {
      console.error('Error deleting template:', err);
      console.error("ERROR:", 'Failed to delete template');
      throw err;
    }
  }, [templates, saveTemplates]);
  ...
  const duplicateTemplate = useCallback(async (templateId: string) => {
    try {
      ...
      console.log("SUCCESS:", 'Template duplicated successfully!');
      return duplicated;
    } catch (err) {
      console.error('Error duplicating template:', err);
      console.error("ERROR:", 'Failed to duplicate template');
      throw err;
    }
  }, [templates, saveTemplates]);
  ...
  const addFollowUpMessage = useCallback(async (
    ...
    try {
      ...
      console.log("SUCCESS:", 'Follow-up message added!');
      return newMessage;
    } catch (err) {
      console.error('Error adding follow-up message:', err);
      console.error("ERROR:", 'Failed to add follow-up message');
      throw err;
    }
  }, [templates, saveTemplates]);
