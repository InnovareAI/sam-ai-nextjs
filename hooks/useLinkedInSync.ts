...
export function useLinkedInSync() {
      ...
      if (accounts.length > 0) {
        ...
        if (accountId) {
          ...
          if (result.success) {
            ...
            console.log("SUCCESS:", 'Auto-sync enabled - messages sync every 5 minutes', {
              duration: 5000
            });
          } else {
  ...
  const performInitialSync = async () => {
    ...
    try {
      ...
      console.error("ERROR:", 'Initial sync failed. Please try again.');
      setSyncState(prev => ({
        ...prev,
  ...
  const performManualSync = useCallback(async () => {
    ...
    try {
      ...
      if (status.messagessynced > 0) {
        console.log("SUCCESS:", `Synced ${status.messagessynced} new conversations`);
      } else {
        console.info("INFO:", 'No new messages to sync');
      }
  ...
  const toggleAutoSync = useCallback(async (enabled: boolean) => {
    try {
      ...
      if (!workspaceId) {
        ...
        console.error("ERROR:", 'Unable to toggle sync - workspace not found');
        return;
      }
      ...
      if (enabled) {
        ...
        if (!unipileRealTimeSync.isConfigured()) {
          ...
          if (accounts.length > 0) {
            ...
            if (accountId) {
              ...
              if (result.success) {
                console.log("SUCCESS:", '☁️ Cloud sync enabled - will continue syncing in background');
              }
            }
          }
        } else {
        ...
        console.info("INFO:", 'Cloud sync disabled');
        setSyncState(prev => ({
          ...prev,
      ...
      console.error("ERROR:", 'Failed to toggle auto-sync');
    }
  }, [backgroundSync]);
