...
export function useLeadValidation(lead: LeadProfile | null, campaign: CampaignProfile | null) {
  ...
  const validateAndAssign = async (onSuccess?: () => void, onError?: (error: string) => void) => {
    ...
    if (state.validationResult.warnings.length > 0) {
      console.warn("WARNING:", `Assignment successful with warnings: ${state.validationResult.warnings.join(', ')}`);
    } else {
      console.log("SUCCESS:", 'Lead successfully assigned to campaign');
    }
...
export function useBulkLeadValidation(leads: LeadProfile[], campaign: CampaignProfile | null) {
  ...
  const validateAndAssignBulk = async (
    ...
    if (validLeadsCount < totalLeadsCount) {
      ...
      console.warn("WARNING:", `${validLeadsCount}/${totalLeadsCount} leads assigned successfully. ${blockedCount} leads blocked.`);
    } else {
      console.log("SUCCESS:", `All ${validLeadsCount} leads successfully assigned to campaign`);
    }

    if (state.validationResult.warnings.length > 0) {
      console.info("INFO:", `Warnings: ${state.validationResult.warnings.slice(0, 2).join(', ')}`);
    }
